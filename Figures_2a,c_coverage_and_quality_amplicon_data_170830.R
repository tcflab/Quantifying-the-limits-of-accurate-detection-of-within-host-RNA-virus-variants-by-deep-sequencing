...
####### FIGURE 2A: QSCORES PLOT AMPLICON DATA #####################################

data = read.table("~/Documents/Graduate_School/CA04_SNP_detection/revisions/data_reanalysis_with_illumina_pipeline/coverage_depth_and_quality_scores/combined.aqhist.amplicon_171221.txt", header=T, sep="\t", col.names=c("id","quality","count","fraction"))
...
####### FIGURES 2C: COVERAGE PLOT AMPLICON #########################################

# read in combined pileup files
data=read.table("~/Documents/Graduate_School/CA04_SNP_detection/revisions/data_reanalysis_with_illumina_pipeline/coverage_depth_and_quality_scores/combined2_amplicon_171221.pileup", header=F, sep="\t", col.names=c("dilution","reference","site","ref_base","coverage"))

data$dilution = gsub(".pileup","",data$dilution)
data["subsampled"] = "no"

data_1000x=read.table("~/Documents/Graduate_School/CA04_SNP_detection/revisions/data_reanalysis_with_illumina_pipeline/coverage_depth_normalization/combined_amplicon_1000x_171221.pileup", header=F, sep="\t", col.names=c("dilution","reference","site","ref_base","coverage"))
...
# now repeat for the 1000x dataframe
site_list = unique(data_1000x$site)
count = 0

for (s in site_list)
{
  df <- data_1000x[data_1000x$site == s,]
  mean_coverage = mean(df$coverage)
  sd_coverage = sd(df$coverage)
  df2 = data.frame(s,mean_coverage,sd_coverage)
...
ggplot(coverage_df, aes(x=s, y=mean_coverage, color=subsampled))+theme(panel.grid.major=element_line(colour=NA,size=NA))+theme(panel.grid.minor=element_line(colour=NA,size=NA))+labs(x="nucleotide site",y="coverage depth")+theme(plot.title=element_text(size=13))+theme(strip.background = element_rect(colour=NA, fill=NA))+theme(axis.line.x=element_line(colour="black"))+theme(axis.line.y=element_line(colour="black"))+theme(strip.text.x=element_text(size=11))+theme(axis.title=element_text(size=13, vjust=0.2))+theme(axis.text=element_text(size=11, colour="black"))+theme(axis.text.x=element_text(angle=45, hjust=1))+theme(legend.text=element_text(size=11))+theme(legend.title=element_text(size=13, face="plain"))+theme(panel.margin=unit(0.9, "lines"))+theme(legend.key.size=unit(0.5, "cm"))+theme(panel.background=element_rect(fill=NA))+theme(legend.key=element_rect(fill=NA))+scale_colour_manual(values=c("grey40","black"))+scale_x_continuous(breaks=seq(0,1800,300), limits=c(0,1800))+geom_ribbon(aes(x=s, ymin=lower, ymax=upper), fill="grey80", linetype=0, alpha=0.6)+geom_line(size=0.7)+scale_y_log10(limits=c(100,1000000), breaks=c(100,1000,10000,100000,1000000))

...
# determine the number of unique polymorphic sites called in the 10^2 to 10^4 amplicon datasets
df_1e1 = data[data$input == "10" & data$method == "amplicon", ]
df_1e2 = data[data$input == "1e+02" & data$method == "amplicon", ]
df_1e3 = data[data$input == "1e+03" & data$method == "amplicon", ]
df_1e4 = data[data$input == "1e+04" & data$method == "amplicon", ]


unique(df_1e1$reference_position)
unique(df_1e2$reference_position)
unique(df_1e3$reference_position)
unique(df_1e4$reference_position)
unique(data$reference_position)
...
df1 = df[df$method =="random_hexamer",]
...
df$method.x[is.na(df$method.x)] <- "random_hexamer"
df$method.y[is.na(df$method.y)] <- "amplicon"


# remove all dilutions 10^4 and less
df = df[df$input == "1e+06" | df$input == "1e+05",]
...
write.table(total_counts_df, file = "~/Documents/Graduate_School/CA04_SNP_detection/revisions/new_figures/amplicon_vs_random_hexamer_total_counts_170901.txt", sep = "\t")
...
not_detected_in_both = not_detected_in_both[c(1:4,6,8)]

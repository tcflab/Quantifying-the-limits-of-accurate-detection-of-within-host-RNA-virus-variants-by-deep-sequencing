...
###############################################################################
# Figures 4c,d: Input copy number determines reproducibility - random hexamer
###############################################################################

#### FIGURE 4C: BAR PLOT OF FRACTION DETECTED 1, 2, OR 3 TIMES ################

# read in the combined dataframe. 
data=read.table("~/Documents/Graduate_School/CA04_SNP_detection/revisions/data_reanalysis_with_illumina_pipeline/combined_vcfs_randomhex_171221.txt", header=T, stringsAsFactors=F, na.string=c(NA,"NA","na"), sep="\t", col.names=c("reference_position","reference","allele","frequency","input","replicate"))
...
data = data[data$input == "1e+06" | data$input == "1e+05",]
...
#### FIGURE 4D: HISTOGRAM OF SNPS DETECTED 1,2, OR 3 TIMES ##################

...
###############################################################################
# Figure 5: Input DNA copy determines SNP frequency estimation
###############################################################################

#### FIGURE 5B: MEAN AND STANDARD DEVIATION OF SNPS DETECTED >1 TIME - random hexamer dataset ##########

# read in the combined dataframe. 
data=read.table("~/Documents/Graduate_School/CA04_SNP_detection/revisions/data_reanalysis_with_illumina_pipeline/combined_amplicon_and_randomhex_vcfs_171221.txt", header=T, stringsAsFactors=F, na.string=c(NA,"NA","na"), sep="\t", col.names=c("reference_position","reference","allele","frequency","input","replicate", "method"))

# remove all dilutions 10^4 and less
data = data[data$input == "1e+06" | data$input == "1e+05" | data$method == "amplicon",]

# keep only random hexamer data
data <- data[data$method == "random_hexamer",]
...
sort(unique(data_nonunique2$i))
data_nonunique2$i <- factor(data_nonunique2$i, levels=c("50","246","408","424","432","436","470","511","515","579","598","608","622","635","695","1112","1653"))
